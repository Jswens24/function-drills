...
let compareNums = (numOne, numTwo) => numOne > numTwo ? numOne : numTwo
...
function faveColorFinder(str) {
  switch (str) {
    ...
    case 'black':
      ...
      break;
    default:
      return 'you need to evaluate your favorite choice.'
  }
}
...
let whatMovie = bestMovie('Sandlot');
...
let loser = 'Glimmer'
/*
  Write a function that is called theEliminator, which takes in two arguments, contestants (which will be an array of strings), and loser (which will be a string).
  The function should loop over the array of contestant names. If the loser string appears in the array, splice it out. Return the new contestants array.
*/

//CODE HERE
function theEliminator(contestantArr, loserStr) {
  for (let i = 0; i < contestantArr.length; i++) {
    if (contestantArr[i] === loserStr) {
...
let elimChecker = theEliminator(contestants, loser);

console.log(elimChecker);
...
const starWarYell = (str) => console.log(str.toUpperCase());

starWarYell(sampleString);
...
function emailCheck(email) {
  ...
  let trimEmail = stringEmail.trim();
...
console.log(emailCheck(' jessicamswens@gmail.com '));
...
let frogPrice = 3;
function frogsForPurchase(amtGold) {
  let amtFrogs = amtGold / frogPrice;
  let roundedFrog = Math.floor(amtFrogs);
  return roundedFrog;
}

let totalFrogs = frogsForPurchase(10);
...
let arrayIsAscending = arrayAscend(sampleArray);

let checkAscending = arrayAscend(goodArray);
